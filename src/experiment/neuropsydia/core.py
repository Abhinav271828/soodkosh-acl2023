...
class Time():
    ...
    def get(self, reset=True):
        ...
        - time
        """
        t = (builtin_time.time()-self.clock)*1000
    ...
    def wait(self, time_to_wait, unit="ms", frequency=60, round_by_frame=True, skip=None):
        ...
        return((builtin_time.time()-t0)*1000)
...
def wait_for_input(time_max=None, flag=0):
    """
    Low level input checker.
    ...
    if time_max is None:
        while loop:
            for event in pygame.event.get():
                if flag < 0 and pygame.key.get_pressed()[pygame.K_RETURN]:
                    return "ENTER"
                if event.type == pygame.KEYDOWN and -0 <= flag < 2:
                    ...
                    if event.key != pygame.K_RSHIFT and event.key != pygame.K_LSHIFT and event.key != pygame.K_RALT:
                        loop = False
                if event.type == pygame.TEXTINPUT and event.text==" " and 0 <= flag < 2:
                    space_flag = True
                    loop = False
                if event.type == pygame.MOUSEBUTTONUP and flag > 0:
                    mouse_flag = True
                    loop = False
                if event.type == pygame.TEXTINPUT and flag < 0:
                    char_flag = True
                    loop = False
    else:
        local_time = Time()
        while loop and local_time.get(reset=False) < time_max:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN and flag < 2:
                    if pygame.key.get_mods() == 577:
                        modifier = "altgrshift"
                        if event.unicode != "" and event.key not in keys["altgrshift"].keys():
                            keys[modifier][event.key] = event.unicode
                    ...
                    if event.key != pygame.K_RSHIFT and event.key != pygame.K_LSHIFT and event.key != pygame.K_RALT:
                        loop = False
                if event.type == pygame.TEXTINPUT and event.text==" " and flag < 2:
                    space_flag = True
                    loop = False
                if event.type == pygame.MOUSEBUTTONUP and flag > 0:
                    mouse_flag = True
    ...
    if time_out == True:
        ...
        # print(event)
        if flag < 0 and char_flag:
            return event.text
        if space_flag and flag < 2:
            return 'SPACE'
        if flag > 0 and mouse_flag:
            return 'MOUSE1'
        try:
...
def response(allow=None, enable_escape=True, time_max=None, get_RT=True, flag = 0):
    """
    ...
    *Dependencies*
    ...
    while True:
        pressed_key = wait_for_input(time_max=time_max, flag = flag)
        # pr = "PRESSED_KEY_" + pressed_key + "_"
        # print(pr)
