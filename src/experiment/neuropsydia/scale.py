...
#        Draw the line
    ...
    if time_max==None:
        ...
        if local_time.get(reset=False) > time_max:
            time_out = True
    if time_out==True:
        cursor_name = 'cursor_blank.png'
        draw_all()
        response = (edges[0]+edges[1])/2
    if reverse == True:
    ...
    pygame.event.set_blocked(pygame.KEYDOWN)
    if get_RT:
        return (response, response_time.get())
    return(response)